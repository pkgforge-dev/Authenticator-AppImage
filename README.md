...
- Search-provider integration works only on Gnome (same as upstream) & it depends on:
  ...
  - Those are the files that need to be in host's `${XDG_DATA_HOME}` for search-provider functionality to work, which you can delete after the app uninstallation:
